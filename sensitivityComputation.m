...
function [dXdphi,dudphi] = sensitivityComputation(sensitivity,X,Xref,theta_r_dot,u,param,theta,dt)

% evaluate the Jacobians
dfdX = grad_f_X_fcn(X,u,dt,param);
dfdX = full(dfdX);    % full() converts sparse matrix to full matrix (see https://se.mathworks.com/help/matlab/ref/full.html)

dfdu = grad_f_u_fcn(X,u,dt,param);
dfdu = full(dfdu);

dhdX = grad_h_X_fcn(X,Xref,theta_r_dot,theta,param);
dhdX = full(dhdX);

dhdtheta = grad_h_theta_fcn(X,Xref,theta_r_dot,theta,param);
